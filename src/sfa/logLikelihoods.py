...
def nlgl_nhn_b(theta, X, y):
    ...
    # LOG-LIKELIHOOD
    loglik = np.sum(-np.log(sigma) + np.log(2.0) + logphi_a + logPhi_w)
...
def lgl_nhn_a(theta, X, y):
    ...
    L = (-0.5 * n * np.log(np.pi / 2.0) - n * np.log(s) + np.sum(logPhi) - 0.5 * np.dot(e, e) / s**2)
...
def nlgl_nexP_b(theta, X, y, n, T):
    ...
    L1 = (-n * np.log(sigma_u) - 0.5 * n * (T - 1) * np.log(2.0 * np.pi * sigma2_v) - 0.5 * n * np.log(T))
    ...
    grad_beta = np.sum((T / sigma2_v) * (EX_mean - B[:, None] * X_mean) + (sqrt_T / sigma_v) * delta[:, None] * X_mean, axis=0,)

    d = residual_ss / T - B**2

    dL_dsigma2_v = np.sum(T * d / (2.0 * sigma2_v**2) + B / (sigma2_v * sigma_u))

    dL_dsigma_v = (2.0 * sigma_v * dL_dsigma2_v - n * (T - 1) / sigma_v + np.sum(delta * (sqrt_T * residual_mean / sigma_v**2 - 1.0 / (sqrt_T * sigma_u))))

    dL_dsigma_u = (-n / sigma_u - np.sum(B / sigma_u**2) + np.sum(delta * sigma_v / (sqrt_T * sigma_u**2)))
    ...
    grad_log_sigma2 = (dL_dsigma_v * dsigma_v_dlog_sigma2 + dL_dsigma_u * dsigma_u_dlog_sigma2)
...
def lgl_nexP_a(theta, X, y, n, T):
    ...
    if theta.size != k + 2:
        raise ValueError(f"theta must contain {k + 2} elements: " f"{k} beta coefficients, log(s_v), and log(s_u).")

    if X.shape[0] != n * T:
        raise ValueError(f"X must have n*T = {n * T} rows, but has {X.shape[0]}.")

    if y.size != n * T:
        raise ValueError(f"y must contain n*T = {n * T} observations, " f"but contains {y.size}.")
    ...
    arg1 = (-n * np.log(s_u) - 0.5 * n * (T - 1) * np.log(2.0 * np.pi * s2v) - 0.5 * n * np.log(T))
...
def nlgl_nhnP_b(theta, X, y, n, T):
    ...
    L1 = (n * np.log(2.0) - 0.5 * n * T * np.log(2.0 * np.pi) - 0.5 * n * (T - 1) * np.log(s2v) - 0.5 * n * np.log(D))
    ...
    grad_log_sigma2 = np.sum(dL_ds2v * ds2v_dlog_sigma2 + dL_ds2u * ds2u_dlog_sigma2)

    grad_eta = np.sum(dL_ds2v * ds2v_deta + dL_ds2u * ds2u_deta)

    grad = -np.concatenate((grad_beta, np.array([grad_log_sigma2, grad_eta,]),))
...
def lgl_nhnP_a(theta, X, y, n, T):
    ...
    arg1 = (n * np.log(2.0) - 0.5 * n * T * np.log(2.0 * np.pi) - 0.5 * n * (T - 1) * np.log(s2v) - 0.5 * n * np.log(sigs2))

    arg2 = (-0.5 / s2v * np.sum(residual_ss - (s2u / sigs2) * residual_sum**2))

    a = (-residual_mean * T * s_u / (s_v * np.sqrt(sigs2)))
...
def nlgl_re_b(theta, X, y, n, T):
    ...
    gamma = np.clip(expit(logit_gamma), 1e-12, 1.0 - 1e-12,)
    ...
    log_likelihood = (-0.5 * n * T * np.log(2.0 * np.pi) -0.5 * n * logdet_omega -0.5 * quad)
    ...
    dq_dlogA_i = (-a * sum_e2 + c * (1.0 + A / d) * sum_e**2)

    dq_dlogB_i = (-c * (1.0 - T * B / d) * sum_e**2)

    score_logA = (-0.5 * n * dlogdet_dlogA -0.5 * np.sum(dq_dlogA_i))

    score_logB = (-0.5 * n * dlogdet_dlogB -0.5 * np.sum(dq_dlogB_i))

    # Chain rule to parametrization B
    score_log_sigma2 = score_logA + score_logB

    score_logit_gamma = (-gamma * score_logA + (1.0 - gamma) * score_logB)

    ng = -np.concatenate((score_beta, np.array([score_log_sigma2, score_logit_gamma,]),))
...
def lgl_re_a(theta, X, y, n, T):
    ...
    arg1 = (-0.5 * n * T * np.log(2.0 * np.pi) -0.5 * n * (T - 1) * np.log(s2v) -0.5 * n * np.log(sig2))

    arg2 = (-0.5 / s2v * np.sum(sse - (s2u / sig2) * esum**2))
